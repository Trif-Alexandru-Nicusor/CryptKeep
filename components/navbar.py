...
def navbar():
    ...
    with ui.left_drawer(bordered = True, elevated = True).props('width=246') as side_bar:
        
        with ui.menu_item(on_click = lambda: ui.navigate.to('/main')).style('width: 100%'):
            ui.label(text = 'CryptKeep').style('font-size: 24px')
            
        with ui.expansion(text = 'Vault', icon = 'enhanced_encryption', value = True).style('width: 100%; font-size: 20px'):
            with ui.menu_item(on_click = lambda: ui.navigate.to(target = '/vault_accounts')).style('width: 100%'):
                ...
                ui.label(text = 'Accounts').style('font-size: 16px; position: relative; right: -10px')
                
            with ui.menu_item(on_click = lambda: ui.navigate.to(target = '/vault_keys')).style('width: 100%'):
                ui.icon(name = 'key', size = '25px')
                ui.label(text = 'Keys').style('font-size: 16px; position: relative; right: -10px')
                
            with ui.menu_item(on_click = lambda: ui.navigate.to(target = '/vault_cards')).style('width: 100%'):
                ui.icon(name = 'credit_card', size = '25px')
                ui.label(text = 'Cards').style('font-size: 16px; position: relative; right: -10px')
                
            with ui.menu_item(on_click = lambda: ui.navigate.to(target = '/vault_personal_data')).style('width: 100%'):
                ui.icon(name = 'person', size = '25px')
                ui.label(text = 'PersonaI Data').style('font-size: 16px; position: relative; right: -10px')
                
        with ui.expansion(text = 'Tools', icon = 'handyman', value = True).style('width: 100%; font-size: 20px'):
            with ui.menu_item(on_click = lambda: ui.navigate.to(target = '/vault_accounts')).style('width: 100%'):
                ui.icon(name = 'password', size = '25px')
                ui.label(text = 'Password Generator').style('font-size: 16px; position: relative; right: -10px')
                
            with ui.menu_item(on_click = lambda: ui.navigate.to(target = '/breach_checker')).style('width: 100%'):
                ui.icon(name = 'security', size = '25px')
                ui.label(text = 'Breach Checker').style('font-size: 16px; position: relative; right: -10px')
                
            with ui.menu_item(on_click = lambda: ui.navigate.to(target = '/audit_log')).style('width: 100%'):
                ui.icon(name = 'history', size = '25px')
                ui.label(text = 'Activity Log').style('font-size: 16px; position: relative; right: -10px')
            ...
            with ui.menu_item(on_click = lambda: ui.navigate.to(target = '/duplicate_finder')).style('width: 100%'):
                ...
                ui.label(text = 'Duplicate Finder').style('font-size: 16px; position: relative; right: -10px')
                
            with ui.menu_item(on_click = lambda: ui.navigate.to(target = '/usage_stats')).style('width: 100%'):
                ui.icon(name = 'insights', size = '25px')
                ui.label(text = 'Usage Stats').style('font-size: 16px; position: relative; right: -10px')
